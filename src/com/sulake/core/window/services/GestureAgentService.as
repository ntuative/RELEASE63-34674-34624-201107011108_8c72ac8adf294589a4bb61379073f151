package com.sulake.core.window.services
{
   ...
   public class GestureAgentService implements IGestureAgentService, IDisposable
   {
      ...
      public function begin(param1:IWindow, param2:Function, param3:uint, param4:int, param5:int) : IWindow
      {
         ...
         {
            this._window = param1;
            this._window.addEventListener(WindowEvent.const_367,this.clientWindowDestroyed);
            this.var_314 = param2;
            this._working = true;
      ...
      protected function operate(param1:TimerEvent) : void
      {
         this.var_1411 *= 0.75;
         this.var_1410 *= 0.75;
         if(Math.abs(this.var_1411) <= 1 && Math.abs(this.var_1410) <= 1)
         {
            this.end(this._window);
         }
         else if(this.var_314 != null)
         {
            this.var_314(this.var_1411,this.var_1410);
         }
      }
      ...
      public function end(param1:IWindow) : IWindow
      {
         ...
         if(this._working)
         {
            if(this._window == param1)
            {
               ...
               {
                  this._window.removeEventListener(WindowEvent.const_367,this.clientWindowDestroyed);
               }
               this._window = null;
               this._working = false;
            }
