package com.sulake.core.runtime
{
   ...
   public final class CoreComponentContext extends ComponentContext implements ICore
   {
      ...
      private static const const_1675:String = "asset-libraries";
      
      private static const const_1669:String = "library";
      
      private static const const_1667:String = "service-libraries";
      
      private static const const_1673:String = "library";
      
      private static const const_1671:String = "component-libraries";
      
      private static const const_1670:String = "library";
      
      private static const const_1674:String = "error_data";
      
      private static const const_1672:String = "error_cat";
      
      private static const const_1668:String = "error_desc";
       
      
      private var var_416:LibraryLoaderQueue;
      
      private var var_885:IEventDispatcher;
      
      private var var_584:uint;
      
      private var var_415:Function;
      ...
      public function CoreComponentContext(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         switch(param2 & 0)
         {
            case Core.const_1381:
               debug("Core; using simple frame update handler");
               this.var_415 = this.simpleFrameUpdateHandler;
               break;
            case Core.const_970:
               debug("Core; using complex frame update handler");
               this.var_415 = this.complexFrameUpdateHandler;
               break;
            case Core.const_941:
               debug("Core; using profiler frame update handler");
               this.var_415 = this.profilerFrameUpdateHandler;
               this.var_231 = new Profiler(this);
               attachComponent(this.var_231,[new IIDProfiler()]);
               var_427.addChild(new ProfilerViewer(this.var_231));
               break;
            case Core.const_499:
               debug("Core; using experimental frame update handler");
               this.var_415 = this.experimentalFrameUpdateHandler;
               break;
            case Core.CORE_SETUP_DEBUG:
               debug("Core; using debug frame update handler");
               this.var_415 = this.debugFrameUpdateHandler;
         }
      }
      ...
      {
         return this.var_584 - this.getNumberOfFilesPending();
      }
      ...
      override public function error(param1:String, param2:Boolean, param3:int = -1, param4:Error = null) : void
      {
         if(param4)
         {
            ErrorReportStorage.setParameter(const_1674,String(param4.getStackTrace()));
         }
         ErrorReportStorage.setParameter(const_1672,String(param3));
         ErrorReportStorage.setParameter(const_1668,param1);
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         this.var_885 = loadingEventDelegate;
         if(this.var_416 == null)
         {
            this.var_416 = new LibraryLoaderQueue(_debug);
         }
         try
         {
            node = config.child(const_1675)[0];
            if(node != null)
            {
               list = node.child(const_1669);
         ...
         {
            error("Failed to parse asset libraries from config xml!",true,Core.const_213,e);
         }
         try
         {
            node = config.child(const_1667)[0];
            if(node != null)
            {
               list = node.child(const_1673);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse interfaces from config xml!",true,Core.const_213,e);
         }
         try
         {
            node = config.child(const_1671)[0];
            if(node != null)
            {
               list = node.child(const_1670);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse components from config xml!",true,Core.const_213,e);
         }
         if(!disposed)
         {
      ...
      private function finalizeLoadingEventDelegate() : void
      {
         if(this.var_885)
         {
            this.var_885.dispatchEvent(new Event(Event.COMPLETE));
         }
      }
      ...
      private function updateLoadingProgress(param1:LibraryLoaderEvent = null) : void
      {
         ...
         if(this.var_885)
         {
            ...
            this.var_885.dispatchEvent(new LibraryProgressEvent(_loc2_.url,param1.bytesLoaded,param1.bytesTotal,_loc2_.elapsedTime));
         }
      }
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(!disposed)
               {
                  if(this.var_885)
                  {
                     this.var_885.dispatchEvent(new LibraryProgressEvent(_loc2_.url,this.var_584 - this.var_416.length,this.var_584,_loc2_.elapsedTime));
                  }
               }
            }
         }
      ...
      override public function registerUpdateReceiver(param1:IUpdateReceiver, param2:uint) : void
      {
         ...
         var _loc3_:int = !!this.var_231 ? 0 : this.var_1649 & 0;
         if(_loc3_ == Core.const_499)
         {
      ...
      private function onEnterFrame(param1:Event) : void
      {
         ...
         this.var_415(_loc2_,_loc2_ - this._lastUpdateTimeMs);
      ...
      private function complexFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         while(priority < CoreComponentContext.const_107)
         {
            ...
            {
               if(this.var_720[priority] < priority)
               {
                  ++this.var_720[priority];
                  skip = true;
               }
            }
            if(!skip)
