package com.sulake.habbo.sound.music
{
   ...
   public class JukeboxPlayListController implements IPlayListController
   {
      ...
      public function JukeboxPlayListController(param1:HabboSoundManagerFlash10, param2:HabboMusicController, param3:IEventDispatcher, param4:IConnection)
      {
         ...
         this.var_787 = [];
         this.var_787.push(new NowPlayingMessageEvent(this.onNowPlayingMessage));
         this.var_787.push(new JukeboxSongDisksMessageEvent(this.onJukeboxSongDisksMessage));
         this.var_787.push(new JukeboxPlayListFullMessageEvent(this.onJukeboxPlayListFullMessage));
      ...
      {
         this.var_253.stop(this.priority);
         this.var_465 = -1;
         this.var_2122 = -1;
         this.var_674 = false;
      }
      ...
      private function onNowPlayingMessage(param1:IMessageEvent) : void
      {
         ...
         this.var_674 = _loc3_.currentSongId != -1;
         if(_loc3_.currentSongId >= 0)
         {
            this.var_253.playSong(_loc3_.currentSongId,HabboMusicPrioritiesEnum.const_138,_loc3_.syncCount / 1000,0,1,1);
            this.var_465 = _loc3_.currentSongId;
         }
         else
         {
            this.stopPlaying();
         ...
         {
            this.var_253.addSongInfoRequest(_loc3_.nextSongId);
         }
         this.var_2122 = _loc3_.currentPosition;
      ...
      private function onJukeboxSongDisksMessage(param1:IMessageEvent) : void
      {
         ...
         while(_loc4_ < _loc3_.songDisks.length)
         {
            ...
            _loc7_ = this.var_253.getSongInfo(_loc5_) as SongDataEntry;
            if(_loc7_ == null)
            {
               _loc7_ = new SongDataEntry(_loc5_,-1,null,null,null);
               if(this.var_886.indexOf(_loc5_) < 0)
               {
                  this.var_886.push(_loc5_);
                  this.var_253.requestSongInfoWithoutSamples(_loc5_);
               }
            }
            _loc7_.diskId = _loc6_;
            this.var_586.push(_loc7_);
            _loc4_++;
         }
      ...
      private function onSongInfoReceivedEvent(param1:SongInfoReceivedEvent) : void
      {
         ...
         {
            _loc4_ = this.var_586[_loc2_];
            if(_loc4_.id == param1.id)
            {
               _loc5_ = _loc4_.diskId;
               _loc6_ = this.var_253.getSongInfo(param1.id) as SongDataEntry;
               if(_loc6_ != null)
               {
                  _loc6_.diskId = _loc5_;
                  this.var_586[_loc2_] = _loc6_;
               }
               break;
            }
            _loc2_++;
         }
         var _loc3_:int = this.var_886.indexOf(param1.id);
         if(_loc3_ >= 0)
         {
