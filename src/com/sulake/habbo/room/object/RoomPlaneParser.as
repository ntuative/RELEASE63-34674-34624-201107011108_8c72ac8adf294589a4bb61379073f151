package com.sulake.habbo.room.object
{
   ...
   public class RoomPlaneParser
   {
      ...
      private static const const_691:Number = 0.25;
      
      private static const const_1231:Number = 0.25;
      
      public static const const_74:int = -110;
      
      public static const const_116:int = -100;
      ...
      public function initializeTileMap(param1:int, param2:int) : Boolean
      {
         ...
         while(_loc3_ < param2)
         {
            ...
            while(_loc7_ < param1)
            {
               _loc4_[_loc7_] = const_74;
               _loc5_[_loc7_] = const_74;
               _loc6_[_loc7_] = false;
               _loc7_++;
            }
      ...
      public function setTileHeight(param1:int, param2:int, param3:Number) : Boolean
      {
         ...
         if(param1 >= 0 && param1 < this.var_220 && param2 >= 0 && param2 < this._height)
         {
            ...
            {
               if(param1 == this._minX || param1 == this.var_451)
               {
                  _loc5_ = false;
                  _loc6_ = this.var_450;
               ...
               if(param2 == this.var_450 || param2 == this.var_452)
               {
                  _loc7_ = false;
                  _loc8_ = this._minX;
                  while(_loc8_ < this.var_451)
                  {
      ...
      public function initializeFromTileData(param1:Boolean = false) : Boolean
      {
         ...
         while(_loc3_ < this._height)
         {
            ...
            while(_loc2_ < this.var_220)
            {
               ...
               {
                  this.var_121[_loc3_][_loc2_] = const_116;
               }
               _loc2_++;
            }
            _loc3_++;
      ...
      private function initialize(param1:Point, param2:Boolean) : Boolean
      {
         ...
         if(param1 != null)
         {
            ...
            this.setTileHeight(param1.x,param1.y,const_74);
         }
         this.var_1020 = getFloorHeight(this.var_121);
         if(!param2)
         {
      ...
      private function createWallPlanes() : Boolean
      {
         ...
         {
            if(this.getTileHeightInternal(_loc8_,_loc9_) > const_116)
            {
               _loc9_--;
               break;
            }
         ...
         if(_loc22_)
         {
            ...
            while(_loc2_ < _loc26_)
            {
               if(!_loc20_[_loc2_])
               {
                  ...
                  {
                     if(this.getTileHeightInternal(_loc29_.x + _loc3_ * _loc30_.x - _loc31_.x,_loc29_.y + _loc3_ * _loc30_.y - _loc31_.y) == const_116)
                     {
                        _loc20_[_loc2_] = true;
                        break;
                     }
      ...
      private function extractTopWall(param1:Point) : Point
      {
         ...
         while(_loc2_ < 1000)
         {
            if(this.getTileHeightInternal(param1.x + _loc2_,param1.y) > const_116)
            {
               return new Point(param1.x + _loc2_ - 1,param1.y);
            }
            if(this.getTileHeightInternal(param1.x + _loc2_,param1.y + 1) <= const_116)
            {
      ...
      private function extractRightWall(param1:Point) : Point
      {
         ...
         while(_loc2_ < 1000)
         {
            if(this.getTileHeightInternal(param1.x,param1.y + _loc2_) > const_116)
            {
               return new Point(param1.x,param1.y + (_loc2_ - 1));
            }
            if(this.getTileHeightInternal(param1.x - 1,param1.y + _loc2_) <= const_116)
            {
      ...
      private function addWall(param1:IVector3d, param2:IVector3d, param3:IVector3d, param4:IVector3d, param5:Boolean, param6:Boolean, param7:Boolean) : void
      {
         ...
         var _loc8_:Number = const_1231 * this.var_1507;
         var _loc9_:Number = const_691 * this.var_998;
         ...
         this.addPlane(RoomPlaneData.const_301,Vector3d.sum(param1,param3),param2,_loc11_,[_loc10_,param4]);
         if(param5)
         {
            this.addPlane(RoomPlaneData.const_301,Vector3d.sum(Vector3d.sum(param1,param2),param3),Vector3d.product(param3,-(param3.length + _loc9_) / param3.length),_loc11_,[_loc10_,param4]);
         }
         if(param6)
         {
            this.addPlane(RoomPlaneData.const_301,Vector3d.sum(param1,Vector3d.product(param3,-_loc9_ / param3.length)),Vector3d.product(param3,(param3.length + _loc9_) / param3.length),_loc11_,[_loc10_,param4]);
            if(param7)
            {
               _loc12_ = Vector3d.product(param2,_loc8_ / param2.length);
               this.addPlane(RoomPlaneData.const_301,Vector3d.sum(Vector3d.sum(param1,param3),Vector3d.product(_loc12_,-1)),_loc12_,_loc11_,[_loc10_,param2,param4]);
            }
         }
      }
      ...
      private function addFloor(param1:IVector3d, param2:IVector3d, param3:IVector3d, param4:Boolean, param5:Boolean, param6:Boolean, param7:Boolean, param8:Number, param9:Number, param10:Number, param11:Number, param12:Boolean, param13:Boolean) : void
      {
         ...
         if(_loc14_)
         {
            _loc18_ = const_691 * this.var_998;
            _loc19_ = _loc18_;
            if(this.var_998 > 1)
            {
               _loc19_ = const_691;
      ...
      public function getXML() : XML
      {
         ...
         var _loc13_:int = 0;
         var _loc14_:* = null;
         var _loc1_:XML = <tileMap width="{this.var_220}" height="{this._height}" wallHeight="{this.var_1508}"/>;
         var _loc2_:int = 0;
         ...
         var _loc4_:int = 0;
         while(_loc4_ < this.var_326.length)
         {
            _loc12_ = this.var_326.getWithIndex(_loc4_);
            if(_loc12_ != null)
            {
               _loc13_ = this.var_326.getKey(_loc4_);
      ...
      public function addFloorHole(param1:int, param2:int, param3:int, param4:int, param5:int) : void
      {
         ...
         this.var_326.add(param1,_loc6_);
      }
      
      public function removeFloorHole(param1:int) : void
      {
         this.var_326.remove(param1);
      }
      ...
      private function initializeHoleMap() : void
      {
         ...
         while(_loc4_ < this.var_326.length)
         {
            _loc5_ = this.var_326.getWithIndex(_loc4_);
            if(_loc5_ != null)
            {
               _loc6_ = _loc5_.x;
