package com.sulake.habbo.room.object.visualization.furniture
{
   ...
   public class FurnitureParticleSystem
   {
      ...
      private var var_1286:Number = 1.0;
      
      private var var_681:BitmapData;
      
      private var var_832:ColorTransform;
      
      private var var_1561:ColorTransform;
      
      private var var_1967:Matrix;
      ...
      public function FurnitureParticleSystem(param1:AnimatedFurnitureVisualization)
      {
         ...
         this.var_1561 = new ColorTransform();
         this.var_1967 = new Matrix();
      ...
      public function setAnimation(param1:int) : void
      {
         ...
         this.var_110 = this.var_1287[param1];
         this.var_1035 = false;
         this.updateCanvas();
      }
      ...
      private function updateCanvas() : void
      {
         ...
         if(this.var_1968 >= 0)
         {
            this.var_293 = this._visualization.getSprite(this.var_1968);
            if(this.var_293 && this.var_293.asset)
            {
               ...
               {
                  this.var_100 = this.var_293.asset.clone();
                  if(this.var_832.alphaMultiplier != 1)
                  {
                     this.var_681 = new BitmapData(this.var_100.width,this.var_100.height,true,4278190080);
                  }
               }
               this.var_2702 = -this.var_293.offsetX;
               this.var_2700 = -this.var_293.offsetY;
               this.var_293.asset = this.var_100;
            }
            if(this.var_100)
            {
      ...
      public function getSpriteYOffset(param1:int, param2:int, param3:int) : int
      {
         ...
         {
            return this.var_110.y * this.var_1286;
         }
         return 0;
      }
      ...
      {
         if(!this.var_110 || !this.var_293)
         {
            return;
         }
         if(this.var_100 && this.var_293.asset != this.var_100)
         {
            this.var_293.asset = this.var_100;
         }
         if(this.var_1035)
      ...
      public function updateAnimation() : void
      {
         ...
         var _loc2_:* = 0;
         var _loc8_:int = 0;
         if(!this.var_1035 && this.var_110.hasIgnited)
         {
            this.var_1035 = true;
         }
         _loc8_ = this.var_1170 * this.var_1286;
         this.var_110.update();
         if(this.var_1035)
         {
            if(this.var_110.roomObjectSpriteId >= 0)
            ...
            {
               this.var_100.draw(this.var_681,this.var_1967,this.var_832,BlendMode.NORMAL,null,false);
            }
            for each(_loc11_ in this.var_110.particles)
            {
               _loc2_ = Number(_loc11_.y);
               _loc3_ = this.var_2702 + (_loc11_.x - _loc11_.z) * 10 / 10 * this.var_1286;
               _loc4_ = this.var_2700 - _loc8_ + (_loc2_ + (_loc11_.x + _loc11_.z) / 2) * 10 / 10 * this.var_1286;
               ...
               if(_loc6_)
               {
                  ...
                  if(_loc11_.fade && _loc11_.alphaMultiplier < 1)
                  {
                     ...
                     this.var_1561.alphaMultiplier = _loc11_.alphaMultiplier;
                     this.var_100.draw(_loc7_,this._translationMatrix,this.var_1561,BlendMode.NORMAL,null,false);
                  }
                  else
                  {
      ...
      public function parseData(param1:XML) : void
      {
         ...
         var _loc23_:* = null;
         var _loc24_:* = null;
         this.var_2701 = parseInt(param1.@size);
         this.var_1968 = !!param1.hasOwnProperty("@canvas_id") ? int(parseInt(param1.@canvas_id)) : -1;
         this.var_1170 = !!param1.hasOwnProperty("@offset_y") ? int(parseInt(param1.@offset_y)) : 10;
         this.var_1286 = this.var_2701 / 64;
         this._blend = !!param1.hasOwnProperty("@blend") ? Number(Number(param1.@blend)) : Number(1);
         this._blend = Math.min(this._blend,1);
         this.var_832.alphaMultiplier = this._blend;
