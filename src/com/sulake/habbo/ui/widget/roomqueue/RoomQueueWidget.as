package com.sulake.habbo.ui.widget.roomqueue
{
   ...
   public class RoomQueueWidget extends RoomWidgetBase
   {
      ...
      override public function registerUpdateEvents(param1:IEventDispatcher) : void
      {
         ...
         param1.addEventListener(RoomWidgetRoomQueueUpdateEvent.const_410,this.onQueueStatus);
         param1.addEventListener(RoomWidgetRoomQueueUpdateEvent.const_633,this.onQueueStatus);
      ...
      override public function unregisterUpdateEvents(param1:IEventDispatcher) : void
      {
         ...
         param1.removeEventListener(RoomWidgetRoomQueueUpdateEvent.const_410,this.onQueueStatus);
         param1.removeEventListener(RoomWidgetRoomQueueUpdateEvent.const_633,this.onQueueStatus);
      }
      ...
      private function onQueueStatus(param1:RoomWidgetRoomQueueUpdateEvent) : void
      {
         ...
         if(param1.isActive)
         {
            this.var_2080 = param1.type;
            this.var_1624 = false;
            this.var_1335 = param1.position;
         }
         else
         {
            this.var_1624 = true;
         }
         this.var_2867 = param1.hasHabboClub;
         this.var_2081 = param1.isClubQueue;
         localizations.registerParameter("room.queue.position","position",this.var_1335.toString());
         localizations.registerParameter("room.queue.position.hc","position",this.var_1335.toString());
         localizations.registerParameter("room.queue.spectator.position","position",this.var_1335.toString());
         localizations.registerParameter("room.queue.spectator.position.hc","position",this.var_1335.toString());
         this.showInterface();
      }
      ...
      private function showInterface() : void
      {
         ...
         {
            switch(this.var_2080)
            {
               case RoomWidgetRoomQueueUpdateEvent.const_410:
                  _loc1_.caption = !!this.var_2081 ? "${room.queue.position.hc}" : "${room.queue.position}";
                  _loc2_.caption = "${room.queue.spectatormode}";
                  _loc3_.visible = this.var_1624;
                  break;
               case RoomWidgetRoomQueueUpdateEvent.const_633:
                  _loc1_.caption = !!this.var_2081 ? "${room.queue.spectator.position.hc}" : "${room.queue.spectator.position}";
                  _loc2_.caption = "${room.queue.back}";
                  _loc3_.visible = false;
            }
            _loc2_.visible = this.var_1624;
         }
         var _loc4_:IWindowContainer = this._window.findChildByName("club_container") as IWindowContainer;
         if(_loc4_ != null)
         {
            _loc4_.visible = !this.var_2867;
         }
      ...
      private function exitQueue(param1:WindowMouseEvent) : void
      {
         ...
         var _loc2_:RoomWidgetRoomQueueMessage = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_760);
      ...
      private function changeQueue(param1:WindowMouseEvent) : void
      {
         ...
         if(this.var_2080 == RoomWidgetRoomQueueUpdateEvent.const_410)
         {
