package com.sulake.habbo.communication.demo
{
   ...
   public class SSOTicket extends EventDispatcher
   {
      ...
      private var var_1672:IHabboWebLogin;
      ...
      public function SSOTicket(param1:IAssetLibrary, param2:IHabboWebLogin, param3:String, param4:Boolean = true)
      {
         ...
         this.var_1671 = "http://" + param3 + "/client";
         if(!param4)
         {
            this.var_1672.init();
            this.var_1672.addEventListener(HabboWeb.const_310,this.requestClientURL);
         }
         else
         {
      ...
      {
         var _loc2_:String = this.var_1671;
         if(this._assets.hasAsset(_loc2_))
         {
            Logger.log("[CoreLocalizationManager] reload localization for url: " + this.var_1671);
         }
         var _loc3_:URLRequest = new URLRequest(this.var_1671);
         if(this._assets.hasAsset(_loc2_))
         {
      ...
      private function clientURLComplete(param1:Event = null) : void
      {
         ...
         {
            this.var_1672.requestReAuthenticate();
         }
         else
         {
            try
            {
               this.var_2176 = /\"sso.ticket\" : \"(.*?)\"/.exec(data)[1];
               this.var_1953 = /\"flash.client.url\" : \"(.*?)\"/.exec(data)[1];
               facebookData = /\"facebook.user\" : \"(.*?)\"/.exec(data);
               if(facebookData)
               {
                  this.var_2993 = Boolean(facebookData[1]);
               }
               if(this.var_2176.length > 0)
               {
