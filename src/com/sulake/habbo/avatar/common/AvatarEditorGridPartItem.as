package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_801:IFigurePartSet;
      ...
      private const const_2353:int = 13421772;
      
      private const const_2354:int = 16777215;
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         this.var_453 = this.var_78.controller.manager.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_747 != null && !this.var_1881)
            {
               _loc3_ = this.var_747;
            }
            else
            ...
            if(this.var_801)
            {
               switch(this.var_801.clubLevel)
               {
                  ...
                  case HabboClubLevelEnum.const_35:
                     _loc2_.style = HabboIconType.const_737;
                     break;
                  case HabboClubLevelEnum.const_43:
                     _loc2_.style = HabboIconType.const_813;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this.var_1517.color = this.const_2353;
         }
         else
         {
            this.var_1517.color = this.const_2354;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_453.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_453.isFigureReady(_loc1_))
         {
            this.var_453.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               ...
               _loc4_ = this.var_453.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_656 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1879)
               {
                  ...
                  this.var_1879 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_656.width,this.var_656.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_656.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_656.y;
