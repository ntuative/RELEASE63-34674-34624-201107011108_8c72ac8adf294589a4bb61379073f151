package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridView
   {
      ...
      public function AvatarEditorGridView(param1:IAvatarEditorCategoryModel, param2:String, param3:IHabboWindowManager, param4:IAssetLibrary)
      {
         ...
         if(this._view != null)
         {
            this.var_636 = this._view.findChildByName("part_thumbs") as IItemGridWindow;
            this.var_531 = new Array();
            this.var_531.push(this._view.findChildByName("palette0") as IItemGridWindow);
            this.var_531.push(this._view.findChildByName("palette1") as IItemGridWindow);
         }
      }
      ...
      public function initFromList() : void
      {
         ...
         var _loc5_:* = null;
         var _loc6_:* = null;
         var _loc1_:CategoryData = this.var_78.getCategoryData(this.var_1467);
         if(!_loc1_)
         {
            return;
         }
         ...
         for each(_loc2_ in _loc1_.parts)
         {
            if(_loc2_)
            {
               this.var_636.addGridItem(_loc2_.view);
         ...
         while(_loc3_ < this.const_728)
         {
            ...
            _loc5_ = this.var_531[_loc3_] as IItemGridWindow;
            if(!(!_loc4_ || !_loc5_))
            {
      ...
      {
         var _loc3_:IWindow = this.var_636.getGridItemAt(param1);
         if(!_loc3_)
         {
            return;
         }
      ...
      private function partEventProc(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         {
            _loc3_ = this.var_636.getGridItemIndex(param1.window);
            this.var_78.selectPart(this.var_1467,_loc3_);
         }
      }
      ...
      private function paletteEventProc(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(param1.type == WindowMouseEvent.const_46)
         {
            ...
            {
               if(this.var_531.length > _loc7_)
               {
                  _loc4_ = this.var_531[_loc7_] as IItemGridWindow;
                  _loc5_ = _loc4_.getGridItemIndex(_loc6_);
                  if(_loc5_ > -1)
                  {
                     this.var_78.selectColor(this.var_1467,_loc5_,_loc7_);
                     return;
                  }
               }
               _loc7_++;
